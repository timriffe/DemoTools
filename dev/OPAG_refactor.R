...
#' @description This method could be useful whenever a reasonable standard is available. At present
#' @examples
...
#' 
#' \dontrun{
#' plot(Age, Pop, type = 'l',xlim=c(0,100))
...
OPAG_simple <- function(Pop, Age, OAnow = max(Age), StPop, StAge, OAnew = max(StAge)){
	...
	# assume single
	stopifnot(test.single(Age))
	stopifnot(test.single(StAge))
