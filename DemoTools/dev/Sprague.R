...
grabill <- function(popmat){
	...
	scmg[m1 + 1, m]   <- 1
	# grabill chunk, need to experiement to graft ends well
	...
	colSums(scmg)
	#scm2              <- scm %*% diag(1 / colSums(scm))
	
	pop1              <- scm %*% popmat
	popg              <- scmg %*% popmat
	
	# weights for grafting in grabill
	w10               <- exp(row(pop1[1:10, ]) ) / exp(10.1)
	popg[1:10, ]      <- w10 * popg[1:10, ] + (1 - w10) * pop1[1:10, ]
	popg[fr:lr, ]     <- w10[10:1, ] * popg[fr:lr, ] + (1 - w10[10:1, ]) * pop1[fr:lr, ]
	
	# now weights for rescaling of grabill to match marginal sums
	wr                <- pop1 * 0 + 1
	wr[1:10, ]        <- w10
	wr[fr:lr, ]       <- w10[10:1, ]
	wr[nrow(wr), ]    <- 0
	...
	redist <- colSums(pop1) - colSums(popg)
	
	grab.middle       <- popg * wr
	have.this         <- colSums(grab.middle)
	# the difference to redistribute
	add.in            <- (grab.middle %*% diag(1/have.this)) %*% diag(redist)
	popg              <- popg + add.in
