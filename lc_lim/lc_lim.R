...
#' Coale-Guo method is allow to smooth rates at older ages.
#'
#' @note Draft Version
#'
#' @param Males numeric. Matrix of abridged age classes by year.
#' @param Females numeric. Matrix of abridged age classes by year. 
#' @param type character. Either rates \code{"m"}, conditionated probabilities \code{"q"} or survival function \code{"l"} included in matrices
#' @param Years numeric. Vector of observed decimal years for input matrix. Same number of columns than input matrices.
#' @param Age numeric. Vector with inferior limit of abridged age classes with data. Same number of rows than input matrix.
#' @param Sex character. Either male \code{"m"}, female \code{"f"}.
#' @param Years_target numeric. Vector of decimal years to interpolate or extrapolate.
#' @param e0_Years numeric. Vector of decimal years where \code{"e_0"} should be fitted when apply method.
#' @param e0_Males numeric. Vector of life expectancy by year to be fitted. Same length than \code{"e0_Years"}.
#' @param e0_Females numeric. Vector of life expectancy by year to be fitted. Same length than \code{"e0_Years"}.
#' @param prev_divergence logical. Whether or not prevent divergence and sex crossover. Default \code{FALSE}.
...
#'   \code{"Coale-Guo"} or other functions in \code{"lt_abridged"} function.
#' @inheritParams lt_a_closeout #???
#' @export
#' @return Lifetable in a data.frame with columns
#' \itemize{
#'   \item{Year}{integer. Years included in Years_target},
...
#' lc_lim_data <- lc_lim(Males, Females, "m", Years, Age, Years_target)
#' 
#' # LC with  limited data and non-divergence btw sex
#' lc_lim_nondiv <- lc_lim(Males, Females, "m", Years, Age, Years_target,
#'                         prev_divergence = T)
#'                         
#' # LC with limited data. rReplicate e0  (see performance issue)
#' lc_lim_fite0 <- lc_lim(Males, Females, "m", Years, Age, Years_target,
#'                        e0_Years = c(1988,1995,2001,2012),
#'                        e0_Males = 77:80, 
#'                        e0_Females = 77:80)
#'                        
#' # LC with limited data. Smoothing older ages rates
#' lc_lim_CoaleGuo <- lc_lim(Males,Females, type = "m", Years, Age, Years_target,
#'                           extrapLaw = "Coale-Guo")

lc_lim <- function(Males = NULL, 
                     Females = NULL, 
                     type = "m", 
                     Years = as.numeric(colnames(Males)), 
  ...
  cm    <- 0
  cm[2] <- d1m <- (ktom[nYears] - ktom[1])/(Years[nYears] - Years[1]) # slope
  cm[1] <- ktom[1] - cm[2] * Years[1] # intercept
  ktm   <- cm[1] + cm[2] * Years_target # interpolate/extrapolate
  k0m   <- cm[1] + cm[2] * Years[1] # first year
  ...
  cf    <- 0
  cf[2] <- d1f <- (ktof[nYears] - ktof[1])/(Years[nYears] - Years[1])
  cf[1] <- ktof[1] - cf[2] * Years[1]
  ktf   <- cf[1] + cf[2] * Years_target
  k0f   <- cf[1] + cf[2] * Years[1]
  
  # ask if prevent divergence and replicate target e0 ---------------------------------------------------------
  
  if(is.null(e0_Years)){ # not rep e0
    
    # basic
    nMxm_hat <- exp(axm + sweep(matrix(bxm, nAge, length(Years_target)),MARGIN=2, ktm,`*`))
    nMxf_hat <- exp(axf + sweep(matrix(bxf, nAge, length(Years_target)),MARGIN=2, ktf,`*`))
    
    # avoid divergence extrapolating to 1950: same bx and kt
    if(prev_divergence == T){
      ...
      nMxm_hat_div <- nMxm[,1] * exp(sweep(matrix(bxm,nAge,length(Years_target)),MARGIN=2,kt-k0,`*`))
      nMxf_hat_div <- nMxf[,1] * exp(sweep(matrix(bxm,nAge,length(Years_target)),MARGIN=2,kt-k0,`*`))
      # only for those years extra to 1950
      Years_extrap <- Years_target<min(Years)
    ...
    e0m = interp_linear(e0_Years,e0_Males,Years_target)
    e0f = interp_linear(e0_Years,e0_Females,Years_target)
    ...
    for(j in 1:nYears_Target){
      ...
      ktm_star[j] <- optimize(f = kt_obj_fun,interval = c(-20,20),
                              ax=axm,bx=bxm,age=Age,sex="m",e0_target=e0m[j])$minimum
    }
    ...
    for(j in 1:nYears_Target){
      ...
      k0 = k0f + d1f * j
      k0 = ifelse(abs(k1)>.1, abs(k1),.1)
      ktf_star[j] <- optimize(f = kt_obj_fun,interval = c(-20,20),
                              ax=axf,bx=bxf,age=Age,sex="f",e0_target=e0f[j])$minimum
    }
  
    # get rates with optim k
    nMxm_hat <- exp(axm + sweep(matrix(bxm,nAge,length(Years_target)),MARGIN=2,ktm_star,`*`))
    nMxf_hat <- exp(axf + sweep(matrix(bxf,nAge,length(Years_target)),MARGIN=2,ktf_star,`*`))
  }
  
  # life tables output ------------------------------------------------------------
  out <- rbind(
    do.call(rbind, apply(nMxm_hat, 2, function(x) {
      lt_abridged(nMx = x, Age = Age, Sex = "m", axmethod = "un",a0rule = "cd")})),
    do.call(rbind, apply(nMxf_hat, 2, function(x) {
      lt_abridged(nMx = x, Age = Age, Sex = "f", axmethod = "un",a0rule = "cd")}))) %>% 
    mutate(Year = rep(sort(rep(Years_target,nAge)), 2), 
